...
function cargarImagenes() {
    fetch("https://randomuser.me/api/?results=6&format=XML")
        ...
        .then(data => { //La función fetch no tiene un analizador XML nativo
            let parser = new DOMParser(); //convierte el texto de la respuesta en un objeto XML 
            let xmlDoc = parser.parseFromString(data, "application/xml"); //convertimos el objeto XML a un objeto Document
            cargarXML(xmlDoc);
            repartirCartas();
        })
...
function cargarXML(xml) {
    ...
    // let docXML = xml.responseXML; //ahora ya tenemos un xml, no hace falta obtener el objeto Document del XML
    let p = xml.getElementsByTagName("results");
    for (let j = 0; j < 2; j++){
        for (let i = 0; i < 6; i++) {
